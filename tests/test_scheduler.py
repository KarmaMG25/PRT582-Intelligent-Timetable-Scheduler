...
def test_scheduler_orders_prerequisite_before_dependent_course():
    ...
    by_course = {
        item["course"]: item
        for item in result["timetable"]
    }

    assert (
        slots.index(by_course["BASIC"]["slot"])
        < slots.index(by_course["ADV"]["slot"])
    )
...
def test_same_inputs_produce_same_timetable():
    ...
    first_result = generate_timetable(
        courses,
        rooms,
        slots,
        availability,
    ...
    second_result = generate_timetable(
        courses,
        rooms,
        slots,
        availability,
...
# TDD refinement 1: invalid preferred slot
def test_unknown_preferred_slot_is_rejected():
    courses = [
        {
            "code": "PRT582",
            "students": 25,
            "lecturer": "Alice",
            "preferred_slot": "Tue 15:00",
            "prerequisite": None,
        }
    ]
...
# TDD refinement 2: duplicate time slots
def test_duplicate_time_slots_are_rejected():
    courses = [
        {
            "code": "A",
            "students": 20,
