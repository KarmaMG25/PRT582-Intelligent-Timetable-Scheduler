...
def validate_inputs(courses, rooms, time_slots, lecturer_availability):
    ...
    for lecturer, slots in lecturer_availability.items():
        if not isinstance(slots, list):
            raise TypeError(
                f"availability for {lecturer} must be a list"
            )
        ...
        if unknown:
            raise ValueError(
                f"unknown time slot(s) for {lecturer}: "
                f"{sorted(unknown)}"
            )
...
def prerequisite_satisfied(course, timetable, slot, time_slots):
    ...
    return (
        time_slots.index(prereq_entry["slot"])
...
def _ordered_slots(course, time_slots):
    ...
    # Reject an explicitly supplied preferred slot that is not valid.
    if preferred is not None and preferred not in time_slots:
        raise ValueError(
            "Preferred slot must exist in the valid time-slot list"
        )
...
def _order_courses_by_prerequisite(courses):
    """Return courses in prerequisite-safe order using a topological sort."""
    ...
    def visit(code):
        ...
        if code in visiting:
            raise ValueError(
                "cyclic prerequisite relationship detected"
            )
...
def generate_timetable(
    ...
    """Generate a deterministic timetable and list unscheduled courses.

    The first valid room and time-slot combination is selected.
    A preferred slot is attempted first when supplied.
